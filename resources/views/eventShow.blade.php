<x-main-layout>
	<div class="m-2 p-2 flex justify-between">
		...
		<div class="flex space-x-2">
			...
			<span class="mx-2">{{ $event->start_date->format('m/d/Y') }}</span>
			<span class="mx-2">{{ $event->end_date->format('m/d/Y') }}</span>
		</div>
	</div>
	<div class="mb-16 flex flex-wrap">
				...
				@auth
					<div class="flex space-x-2 p-4" x-data="{
						...
						<button type="button" @click="onHandleLike"
						        ...
						        :class="eventLike ? 'bg-red-700 hover:bg-red-800' : 'bg-slate-400 hover:bg-slate-500 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800'">
						...
						<button type="button" @click="onHandleSavedEvent"
						        ...
						        :class="savedEvent ? 'bg-yellow-700 hover:bg-yellow-800' : 'bg-slate-400 hover:bg-slate-500 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800'">
							<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
							     stroke="currentColor" class="w-3.5 h3.5 mr-2">
								<path stroke-linecap="round" stroke-linejoin="round"
						...
						<button type="button" @click="onHandleAttending"
						        class="text-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center"
						        :class="attending ? 'bg-indigo-700 hover:bg-indigo-800' : 'bg-slate-400 hover:bg-slate-500 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800'">
							Attending
		...
		<div class="w-full shrink-0 grow-0 lg:w-6/12 lg:pl-6 bg-slate-50 rounded-md p-2">
			<p class="mb-6 text-sm text-yellow-900 dark:text-neutral-900">
			...
			<p class="mb-6 mt-4 text-neutral-500 dark:text-neutral-900">
				{{ $event->description }}
			</p>
			<div class="flex justify-start">
				<div class="flex flex-col">
					<div class="mb-4 flex items-center text-sm font-medium">
						...
						<div class="text-yellow-900">{{ $event->country->name }}, {{ $event->city->name }}</div>
					</div>
							...
							<div class="w-full p-4 duration-500">
								<div class="flex items-center rounded-lg bg-white p-4 shadow-md shadow-indigo-50">
									<div>
										...
										<p class="text-sm font-semi-bold text-gray-400">{{ $comment->content }}</p>
										@can('view', $comment)
